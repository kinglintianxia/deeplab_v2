...
if is_server
  if strcmp(dataset, 'voc12')
    VOC_root_folder = '/home/king/Documents/image/VOC2012_orig';
  elseif strcmp(dataset, 'nyu')
    VOC_root_folder = '/home/king/Documents/image/nyu';
  else
    error('Wrong dataset');
  end
else
...
output_mat_folder = fullfile('/home/king/Documents/deeplab_v2/exper', dataset, feature_name, model_name, testset, feature_type);
fprintf(1, 'output_mat_folder: %s\n', output_mat_folder);
save_root_folder = fullfile('/home/king/Documents/deeplab_v2/exper', dataset, 'res', feature_name, model_name, testset, feature_type, post_folder);
...
elseif strcmp(dataset, 'nyu')
  seg_res_dir = [save_root_folder '/results/nyu/'];
  seg_root = fullfile(VOC_root_folder, '');
  gt_dir   = fullfile(VOC_root_folder, '', 'nyu_labels');
end

save_result_folder = fullfile(seg_res_dir, 'SegmentationClass', [id '_' testset '_cls']);
...
elseif strcmp(dataset, 'nyu')
  VOCopts = GetVOCopts(seg_root, seg_res_dir, trainset, testset, 'nyu');
end
...
if is_mat
  ...
  for i = 1 : numel(output_dir)
    ...
    elseif strcmp(dataset, 'nyu')
      img = imread(fullfile(VOC_root_folder, 'nyu_images', [img_fn, '.jpg']));
    end
    
    img_row = size(img, 1);
    img_col = size(img, 2);
    %fprintf(2, 'img_row/img_col: %d/%d\n', img_row,img_col);
